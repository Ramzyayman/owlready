...
--SETTINGS:
local colaTruckID = 29144 --VIN of the coke truck --16142
local colaTruckTrailerID = 29145 --VIN of the coke trailer --16138
...
local debugXmas = true --debug mode true/false

--globals
local colaTruck = true
local colaTruckTrailer = true
local santa = true
local santaTimer = true
local colaDrinkers = {}
...
function isItChristmas()
	...
	if(realtime.month == 7) then --and realtime.monthday > 21 --December
		return true
	end
	return false
end
...
function initiateSanta()
	if isItChristmas() then
		...
		local time = math.random(250*minWait,250*maxWait) --between 50 and 120 minutes
		...
		if debugXmas then
			outputDebugString("xmas: Santa will go to work in "..tostring(math.floor(time/250)).." minutes.")
		end
		initializeDrinking()
	end
end
...
function forceSanta(thePlayer, commandName)
	if(exports.integration:isPlayerScripter(thePlayer)) then
		if santa then
			...
			outputChatBox("Sorry. Santa is at the north polee!",thePlayer,255,0,0)
		end
	end
end
...
function getSantaWait(thePlayer, commandName)
	if(exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerLeadAdmin(thePlayer)) then
		if santa then
			...
			outputChatBox("Sorry. Santa is at the north poleee!",thePlayer,255,0,0)
		end
	end
end
...
function fixSanta(thePlayer, commandName)
	if(exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerAdmin(thePlayer)) then
		if santa then
			...
			outputChatBox("Sorry. Santa is at the north poleeee!",thePlayer,255,0,0)
		end
	end
end
